# === IMPORT LIBRARIES ===
# ALWAYS CHECK IF TIME SHIFT IS WORKING (WEATHER DATA IS IN UTC) SO AS TO PLOT THE REALISTIC SOLAR PV HOURS AS A FACTOR OF ACTUAL IRRADIANCE/DAYLIGHT
...
def create_strategic_plot(data, time_period='daily', filename='strategic_plot'):
    ...
    colors = {
        ...
        'load_shift': '#FF69B4',   # Pink for load shift periods (REMOVED from background)
    ...
    # Highlight strategic periods with background colors - REMOVED LOAD SHIFT SHADING
    for hour_range, color, alpha, label in [
        ...
        # LOAD SHIFT HOURS BACKGROUND SHADING REMOVED
    ]:
    ...
    plt.setp(ax1.xaxis.get_majorticklabels(), rotation=5, ha='center', fontname='Garamond', fontsize=14)
    ...
    plt.setp(ax2.xaxis.get_majorticklabels(), rotation=5, ha='center', fontname='Garamond', fontsize=14)
...
# === ADDITIONAL DAILY PLOTS FOR DECEMBER 3, 4, 5 ===
december_days = ['2024-12-03', '2024-12-04', '2024-12-05']
for day in december_days:
    day_data = df[df.index.normalize() == pd.to_datetime(day)]
    if len(day_data) > 0:
        filename = f"december_{day[-2:]}_strategic"
        fig = create_strategic_plot(day_data, time_period='daily', filename=filename)
        plt.savefig(f"{filename}.pdf", format='pdf', bbox_inches='tight', dpi=300)
        plt.show()
        plt.close(fig)
...
# Generate plots for key weekly periods
plot_periods = [
    ('2024-01-02', '2024-01-05', 'January First Week', 'daily'),
    ('2024-07-02', '2024-07-05', 'July First Week', 'daily'),
    ('2024-12-02', '2024-12-05', 'December First Week', 'daily')
]

for start_date, end_date, period_name, time_period in plot_periods:
    plot_data = df[(df.index >= start_date) & (df.index < end_date)]
    if len(plot_data) > 0:
        filename = f"{period_name.lower().replace(' ', '_')}_strategic_FINAL_2024"
        fig = create_strategic_plot(plot_data, time_period, filename)
        # Save as PDF
        plt.savefig(f"{filename}.pdf", format='pdf', bbox_inches='tight', dpi=300)
        plt.show()
...
print("All strategic plots have been saved as PDF files!")
